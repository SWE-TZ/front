    ...
    return (
        <div className='grid grid-cols-2 w-[540px] h-[370px] border-2 border-dark rounded-xl mb-[50px] container mx-auto max-w-screen-sm shadow-2xl'>
            <div className="col-start-1 ml-[20px]">
                <Image src={employee?.photo || img} width={100} height={100} className="rounded-full h-[170px] w-[170px]  mt-[20px] border-2 border-dark" />
            ...
            <div className="col-start-2 mt-[20px] ml-[-30px]">
                ...
                <label className="text-dark font-bold text-[20px] block"><FontAwesomeIcon icon={faStar} className="text-sm pr-[5px] pb-[3px]"/>{employee?.rating || "rating"}<span className="text-[15px] pl-[10px] inline-block">({employee?.reviews_num || 'reviews'} reviews)</span></label>
                <label className="text-dark font-semibold text-[20px] mb-[3px] block ">{employee?.category || "category"}</label>
