...
import { usePathname } from "next/navigation"; // Modern way to track route changes
...
export default function Nav() {
  ...
  const pathname = usePathname(); // Tracks route changes dynamically
  ...
  useEffect(() => {
    ...
    // Trigger check on route changes (modern approach)
    checkLoginStatus();
  ...
  const handleSignOut = () => {
    ...
    setIsLoggedIn(false);
    window.location.href = "/pages/choose"; // Use window.location for navigation
  };
      ...
      <div className="h-full container mx-auto max-w-screen-xl flex items-center justify-between">
        ...
        {isLoggedIn ? (
          <div className="flex items-center space-x-6">
            ...
            <button
              className="bg-dark text-light font-semibold py-3 px-6 rounded-full shadow-md hover:bg-light hover:text-dark transition-colors duration-300 transform hover:scale-105"
