...
useEffect(() => {
  ...
  function filterEmployees() {
    ...
    if (filters.rating) {
      filtered = filtered.sort(
        (b, a) => parseFloat(a.rating) - parseFloat(b.rating)
      );
    }
    ...
    if (filters.sat_sun) {
      filtered = filtered.filter(
        (employee) =>
          employee.available_days.includes("Saturday") ||
          employee.available_days.includes("Sunday")
      );
    }
    ...
    if (filters.mon_tue) {
      filtered = filtered.filter(
        (employee) =>
          employee.available_days.includes("Tuesday") ||
          employee.available_days.includes("Monday")
      );
    }
    ...
    if (filters.wed_thu) {
      filtered = filtered.filter(
        (employee) =>
          employee.available_days.includes("Wednesday") ||
          employee.available_days.includes("Thursday")
      );
    }

    if (filters.fri) {
      filtered = filtered.filter((employee) =>
        employee.available_days.includes("Friday")
      );
    }
    ...
    if (filters.time) {
      if (time.includes("AM")) {
        filtered = filtered.filter(
          (employee) =>
            parseInt(employee.start_time) <= parseInt(time.slice(0, 2).trim())
        );
      } else {
        filtered = filtered.filter(
          (employee) =>
            parseInt(employee.end_time) > parseInt(time.slice(0, 2).trim())
        );
      }
    }
    ...
    if (filters.price_range) {
      filtered = filtered.filter(
        (employee) => parseInt(employee.price) < parseInt(price)
      );
    }
  ...
  const updateFilter = (filterName) => {
    setFilters((prevFilters) => {
      ...
      if (filterName != "reset") {
        const newfilters = {
          price: isPriceOrRating
            ? filterName === "price"
              ? !prevFilters.price
              : false
            : prevFilters.price,
          rating: isPriceOrRating
            ? filterName === "rating"
              ? !prevFilters.rating
              : false
            : prevFilters.rating,
          ...
          price_range:
            filterName === "price_range" ? true : prevFilters.price_range,
  ...
  return (
    <div className="bg-light ">
      ...
      <div className="container max-w-screen-xl mx-auto flex grid grid-col-3 gap-4 pt-[50px] bg-light">
        <div className=" justify-center flex ">
          ...
          <Menu>
            ...
            >
              <MenuItem>
                <button
                  className="group flex w-full items-center gap-2 rounded-lg py-1.5 px-3 data-[focus]:bg-dark data-[focus]:text-light"
              ...
              <div className=" justify-center flex ">
                <Menu>
                  <MenuButton className="inline-flex gap-2 w-[150px] rounded-full hover:bg-dark py-1.5 px-3 text-sm/6 font-semibold text-dark border-2 hover:border-light hover:text-light border-dark ">
                    <span className="justify-self-center w-[150px]">
                      PRICES
                    </span>
                  </MenuButton>
        ...
        <div className="flex justify-center items-center py-4 shadow-lg rounded-lg">
          <button
