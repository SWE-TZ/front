...
const ShoppingCart = () => {
  ...
  const data = searchParams.get('employee');

  // Decode and parse the JSON object
  const employee = data ? JSON.parse(decodeURIComponent(data)) : notFound();

  return (
    <div className="relative min-h-screen">
      <Header />
      <ProgressBar currentStep={4} onStepClick={null} />
      <div className="relative flex flex-col lg:flex-row pb-20 w-11/12 mx-auto justify-between space-y-8 lg:space-x-8">
        ...
        <div className="w-full lg:w-1/2 border-dark p-6 rounded-lg shadow-2xl border-double border-4 border-dark">
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <h3 className="text-xl font-semibold mt-2">Total: ${parseInt(employee.price)+2.88}</h3>
          </div>
        ...
        <div className="w-full lg:w-1/3 shadow-2xl p-6 rounded-lg border-double border-4 border-dark">
          ...
          <div className="flex items-center justify-between mb-6 border-b pb-4">
            ...
            <div className="flex-1 ml-4">
              <h4 className="font-medium text-lg">{employee.category}</h4>
              <p className="block mb-2 text-gray-600 flex items-center "><MdDateRange className="mr-2 text-gray-500"/>{employee.available_days} </p>
              <p className="block mb-2 text-gray-600 flex items-center"><Place className="mr-2 text-gray-500"/>{employee.location}</p>
              <p className="block mb-2 text-gray-600 flex items-center"><DesignServices className="mr-2 text-gray-500"/>Large Est.3hr</p>
              <p className="block mb-2 text-gray-600 flex items-center"><Badge className="mr-2 text-gray-500"/>Tasker Name:{employee.name} </p>
            </div>
